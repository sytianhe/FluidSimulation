...
For our principle experimental run, we simulated forty different shapes and for each shape performed ten drops.   
Some date from preliminary trials is given in the Data directory.   Movies for a few of these trials can be found in the Artifacts Directory.  

We will not stress the precise numerical details of the simulation (as they really are just preliminary).  But it is worth observing that the various trials demonstrates two principle modes of behavior: fluttering and tumbling.    In two dimensions, these are the only modes of falling. Which mode a given shape lies in has dramatic  implications on 
