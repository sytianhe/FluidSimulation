...
(Note: This biological overview is an abridged  version of that described in our original project proposal. )
\\ 
\\
...
Our fluid simulation implementation is based on the source code provided for smoke simulation and control. Though not entirely relevant to our  goal of simulating seed dispersion, we began by implementing various features of the smoke control project.  Followed the  \cite{fattal2004}, we included a gathering force $G$ to make the smoke converged to keyframe density:
...
The smoke simulation is subject to significant numerical dissipation.  To eliminate this undesirable affect, we impose a simple normalization condition to maintain the density sum equal to the sum of keyframe density.  Specifically,  we require that the total mass of smoke  in the scene is equal to the total mass of smoke density in the keyframe.  If not, we scale our original density until the target density mass is met:
	$$\rho_{ij} = \rho_{ij}  +  \alpha \sum (\rho_{ij}^* - \rho_{ij} ) $$
In practice we use the modified normailization expression :
	$$\rho_{ij} = \rho_{ij}  + \alpha  (\rho_{ij}^* - \rho_{ij}  )$$
which actively destroys smoke density not matching the key frame.  We find that this destructive feature actually nicely balances the  effects of numerical diffusion. 

For simplicity, our simulation runs on a regular old-fashioned $N\times N$ lattice.  To improve numerical accuracy, we employ a sudo-MAC grid approach to calculate derivatives at cell boundaries as need.  That is, we compute derivatives that we would usually associate with the secondary grid of the MAC grid.  
